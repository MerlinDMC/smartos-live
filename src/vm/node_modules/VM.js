        ...
        loadMetadata(vmobj, function (error, metadata, tags) {
            ...
            for (snap in raw_snapshots) {
                ...
                if (matches && matches[1]) {
                    ...
                    VM.logd.debug('ignoring unfriendly ' + snap.name);
                    continue;
                }
            }
        ...
        if (payload.add_nics.hasOwnProperty(nic)) {
            ...
            if (n.ip != 'dhcp' && n.ip != 'dhcp6') {
                fs.writeFileSync(payload.zonepath + '/root/etc/hostname.'
                    + n.interface, n.ip
                    // if ip has CIDR notation ignore netmask setting
        ...
        function (cb) {
            ...
            for (n in newobj.nics) {
                ...
                filename = newobj.zonepath + '/root/etc/hostname.'
                    + n.interface;
                if (n.ip !== 'dhcp') {
                    fs.writeFileSync(filename, n.ip
                        // if ip has CIDR notation ignore netmask setting
                        + (n.ip.match('\/[0-9]+$') ? '' : ' netmask '
                            + n.netmask)
                        + ' up' + '\n');
                } else {
                ...
                if (n.hasOwnProperty('primary')) {
                    ...
                    if (n.ip === 'dhcp6') {
                        fs.writeFileSync(filename, 'inet6');
                    }
                }
            }
